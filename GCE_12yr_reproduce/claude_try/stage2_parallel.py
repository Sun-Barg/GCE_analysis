...
def _worker_init(work_dir):
    ...
    print(f"  [worker PID={os.getpid()}] init done", flush=True)
...
def _fit_bin_task(task):
    ...
    print(f"  [{time.strftime('%H:%M:%S')}] [PID {pid}] {model} bin {ebin:2d} done in "
          f"{elapsed/60:.1f}min  logL={log_prob_flat[idx_max]:.1f}  flux={flux_best:.3e}",
          flush=True)
...
def run_parallel(target_models=None, n_workers=None, n_ebins=14):
    ...
    print(f"[{time.strftime('%H:%M:%S')}] STAGE 2 PARALLEL :: {len(tasks)} tasks on "
          f"{n_workers} workers  ({len(target_models)} models × {n_ebins} bins)", flush=True)

    t_start = time.time()
    ctx = get_context('fork')
    with ctx.Pool(processes=n_workers, initializer=_worker_init,
                  initargs=(WORK_DIR,)) as pool:
        results = []
    ...
    print(f"\n[{time.strftime('%H:%M:%S')}] all {len(tasks)} tasks done in {total:.1f} min "
          f"(wall clock)", flush=True)
    print(f"  average per task: {np.mean([r['elapsed_sec'] for r in results])/60:.1f} min", flush=True)
    ...
    for m in target_models:
        ...
        out_dat = os.path.join(OUT_DIR, f'GCE_model_{m}_haebarg_v_claude.dat')
        np.savetxt(out_dat,
                   np.column_stack([E, flux_best, flux_std, flux_lo, flux_hi]),
                   header='E[GeV]  E2dN/dE_best  std  lower_1sigma  upper_1sigma')
        out_logL = os.path.join(OUT_DIR, f'GCE_model_{m}_haebarg_v_claude_logL.txt')
        np.savetxt(out_logL, max_logL, header='per-bin max log-likelihood')
        out_pkl = os.path.join(OUT_DIR, f'GCE_model_{m}_haebarg_v_claude.pkl')
        ...
        print(f"  saved: {out_dat}  (-2 log L = {-2.0*max_logL.sum():.2f})", flush=True)
