...
`results_12yr/`. Designed to be re-launchable: a second invocation
...
State:
    ...
    - `results_12yr/*.dat`   completion marker (per model)
    - `log_{M}.txt`          per-model log (append on retry, timestamp banner)

Resume semantics:
    - already in `results_12yr/`           -> skip
    - already in work dir (.dat present)   -> mv to results_12yr/, mark done
...
# rank study: 셀 출력 suffix (production은 '' -> 동작 불변; worker의 RANK_SUFFIX와 동일)
...
def _model_files(model):
    ...
    return [f'GCE_model_{model}{FRONT}_12yr_cholis{RANK_SUFFIX}{ext}' for ext in RESULT_EXTS]


def is_complete(model, results_dir, work_dir):
    """Completion = .dat exists in either results_dir or work_dir."""
    dat = f'GCE_model_{model}{FRONT}_12yr_cholis{RANK_SUFFIX}.dat'
...
def cleanup_stale_intermediates(models, work_dir, results_dir):
    ...
    model_dir = os.path.join(work_dir, 'GC_analysis_DR2', 'Model')
    fits_dir  = os.path.join(work_dir, 'GC_analysis_DR2')
    cwd_dir   = work_dir   # final .dat lives in cwd, not GC_analysis_DR2/
    ...
    for m in models:
        ...
        for convol_suffix in ['', '_no_convol']:
            path = f'{fits_dir}/GC_Extended_srcmap_12yr_front_clean_model_{m}{convol_suffix}.fits'
            _verify_or_delete(path,
        ...
        for comp in ['pion', 'bremss', 'ics']:
            for convol_suffix in ['', '_no_convol']:
                path = f'{fits_dir}/GC_{comp}_model{m}_12yr_front_clean{convol_suffix}.fits'
                _verify_or_delete(path,
                                  lambda p=path: verify_cube(p, expected_xy=(600, 600)),
                                  stats)

        # --- Final .dat + companions ---
        dat_path = os.path.join(cwd_dir, f'GCE_model_{m}_front_12yr_cholis.dat')
        if os.path.exists(dat_path):
            ...
            if ok:
                ...
                for ext in ['.dat', '_fit.npz', '_likelihood_value']:
                    p = os.path.join(cwd_dir, f'GCE_model_{m}_front_12yr_cholis{ext}')
                    if os.path.exists(p):
                        try:
                            os.remove(p)
                            stats['deleted'] += 1
    ...
    shared_paths = [
        f'{fits_dir}/GC_{src}_model_12yr_front_clean{conv}.fits'
...
def main():
    ...
    ap.add_argument('--results-dir',    type=str,   default='results_12yr')
    ...
    try:
        while True:
            ...
            runnable = [m for m in not_done
                        if m not in running and m not in permanent_failed]
