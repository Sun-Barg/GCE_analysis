...
Run from: ~/GCE-Chi-square-fitting/GCE_12yr_reproduce/   (working directory)
Usage:
...
16 steps:
    ...
    10. SourceList XML        -> Model/GC_model_DR2.xml
    11. Prune iso/galactic    -> Model/GC_psc_model_DR2.xml
    12. Classify sources      -> Model/source_classification.npz (sig + not_sig)
    13. Main psc mask         -> Model/GC_mask_60x60_definitions_DR2.npy
...
EXTENDED_DIR    = '../GCE_12yr_data/Templates/'
...
PHOTON_LIST_FILE   = 'photon_data_12yr.txt'
...
SC_LIST_FILE   = 'sc_files_12yr.txt'

# Time window: 'INDEF' uses the full spacecraft time range
# Cholis fermi_w009_to_w670 alignment (2026-06-16):
#   TMIN = 239557417 = w009 TSTART (already Cholis-matched)
#   TMAX = 639533545 = w670 TSTOP  (was 634027417 ~12.5yr, cut ~9 weeks short of w670)
TMIN = 239557417
TMAX = 639533545
...
ALLSKY_SELECT   = f'{WORK_DIR}/Allsky_select_12yr{front}_clean.fits'
ALLSKY_GTI      = f'{WORK_DIR}/Allsky_gti_12yr{front}_clean.fits'
GC_CCUBE        = f'{WORK_DIR}/GC_ccube_12yr{front}_clean.fits'
ALLSKY_LTCUBE   = f'{WORK_DIR}/Allsky_ltcube_12yr{front}_clean.fits'
GC_EXPCUBE_CTR  = f'{WORK_DIR}/GC_expcube_center_12yr{front}_clean.fits'
ALLSKY_EXP_EDGE = f'{WORK_DIR}/Allsky_expcube_edge_12yr{front}_clean.fits'
BIN_DEF_FILE    = f'{WORK_DIR}/bin_definitions.fits'

MODEL_DIR              = f'{WORK_DIR}/Model'
GC_MODEL_XML           = f'{MODEL_DIR}/GC_model_DR2.xml'
GC_PSC_MODEL_XML       = f'{MODEL_DIR}/GC_psc_model_DR2.xml'
SOURCE_CLASSIFICATION  = f'{MODEL_DIR}/source_classification.npz'
PSC_MASK_NPY           = f'{MODEL_DIR}/GC_mask_60x60_definitions_DR2.npy'
...
def build_source_classification():
    """Parse GC_psc_model_DR2.xml + FL16Y catalog -> two lists of
...
def build_steps():
    ...
    return [
        ...
        Step(11, 'Prune iso/galactic -> GC_psc_model_DR2.xml',
             [GC_PSC_MODEL_XML],
...
def main():
    ...
    if not os.path.isdir(WORK_DIR):
        _abort(f'{WORK_DIR} not found; run from the working directory '
               f'(~/GCE-Chi-square-fitting/GCE_12yr_reproduce/).')
