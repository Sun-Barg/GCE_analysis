...
> ⚠️ **CONSEQUENCE**: the existing 80-model results were produced with
> the BUGGY template (integral 0.7259, amplitude distorted) and are
> **INVALID** — likely the root of the Model X rank 1→27 regression.
> After regenerating the template, the **full 80-model main fit must be
> rerun**. cov (22 ROI) used the correct normalization via
> `make_wimp_map_per_roi.py` and is unaffected, but main↔cov relative
> scale must be re-checked once the main fit is rerun.
...
multiplicative factor 1e-3/E_GeV²), the mask convention V44
(`np.flip(mask, axis=2)`) is consistently applied to a non-flipped
MapCube, and fit results are uncompromised by spatial orientation. The
true root cause of (β) ranking inversion remains unidentified and must
be sought in other stages (gtsrcmaps, fit dynamics, GALPROP raw
model-to-model spectrum variation interacting with the fit landscape).
...
- `results_17yr/*.dat`: ⚠️ **INVALIDATED 2026-05-19** — the 80/80 main
  fit completed, but was produced with the BUGGY GCE_template_NFW2.fits
  (normalization integral 0.7259 instead of 1.0; see make_gce_template.py).
  Template regenerated 2026-05-19 (integral 1.0, cov-consistent).
  **Full 80-model main fit must be rerun with the fixed template.**
  (The launcher_watchdog infrastructure itself was validated: 14
  restarts, zero data loss — only the input template was wrong.)
...
### Known characteristic (not a bug): c_iso → 0 at 1–10 GeV

For 7/9 main models c_iso sticks near the [0,∞) boundary at 1–10 GeV.
Decomposition (12yr V48): Cash Poisson ~3.4e5/bin vs chi²_iso dynamic
range ~10 → external constraint effectively dead vs Poisson dominance.
Reproduces 12yr V48. 해밝 accepted V48 conclusion (proceed): c-coeffs
are systematic-uncertainty indicators, not physical GDE calibration;
GCE flux envelope robust to c-swap. Rejected: chi² weight increase /
