...
Changes:
  [fb17-v1] (2026-07-27) EXPECTED_NEBINS env-driven: FB17=1 -> 17
      (front+back extended bins), GCE_NEBINS=<n> explicit override,
      default 14 (fiducial, unchanged). All verify_* defaults follow
      (bind at import). masking() itself is bin-count agnostic
      (Table III interp1d + extrapolate + clamp>=0) and needs no
      change for bins above 51.9 GeV.
"""
...
# EXPECTED_NEBINS: bin-count contract for all verify_* defaults.
#   default             -> 14 (Cholis fiducial, FRONT-only)
#   FB17=1 (env)        -> 17 (front+back, 3 extra high-E bins)
#   GCE_NEBINS=<n> (env) -> explicit override (takes precedence)
# Default arguments bind at import time, so the env must be set BEFORE
# any pipeline process starts (wrapper subprocesses inherit os.environ).
_FB17_ENV = bool(os.environ.get('FB17', '').strip())
EXPECTED_NEBINS = int(os.environ.get('GCE_NEBINS', '17' if _FB17_ENV else '14'))
