...
Memory constraint:
    ...
    analysis server (128 GB total, ~30 GB system reserve).
    DR4 variant reuses templates (no gtsrcmaps); 22 workers is safe - prior full runs peaked ~20 GB. Monitor `free -h`.
...
FRONT          = '_front'
RUNNER_SCRIPT  = 'run_one_roi_cov_wrapper_DR4.py'
LOCK_FILE      = '.launcher_cov_DR4.pid'
...
def _roi_files(roi):
    return [f'GCE_cov_l{roi}{FRONT}_17yr_cholis_DR4{ext}' for ext in RESULT_EXTS]


def is_complete(roi, results_dir, work_dir):
    dat = f'GCE_cov_l{roi}{FRONT}_17yr_cholis_DR4.dat'
...
def main():
    ...
    ap.add_argument('--workers',        type=int,   default=22,
                    help='max concurrent ROI subprocesses (default: 22; DR4 = mcmc-only, templates reused)')
    ...
    if not os.path.exists(RUNNER_SCRIPT):
        ...
        sys.exit(2)
    if args.workers > 28:
        print(f"[warn] --workers {args.workers} exceeds the configured ceiling of 28.")
        print(f"       monitor `free -h` carefully; DR4 reuses templates (no gtsrcmaps); prior 22-worker full runs peaked ~20 GB total.")
