...
#        a. mv all workdir orphan .dat (+ companions) into results_17yr/
#        b. rm stale .launcher.pid
#        c. respawn launcher with the canonical command
#   3. Auto-exit on:
#        a. 80/80 .dat in results_17yr/ (success)
...
count_dat() {
    ls "$RESULTS_DIR"/GCE_model_*_front_17yr_cholis.dat 2>/dev/null | wc -l
}
...
recover_orphans() {
    ...
    local moved=0
    shopt -s nullglob
    for dat in GCE_model_*_front_17yr_cholis.dat; do
        local m=${dat#GCE_model_}
        m=${m%_front_17yr_cholis.dat}
        for ext in .dat _fit.npz _likelihood_value; do
            local src="GCE_model_${m}_front_17yr_cholis${ext}"
