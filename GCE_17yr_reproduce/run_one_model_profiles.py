...
Changes vs cell 30:
  ...
  5. multiprocessing Pool(processes=4) — for 16-way parallel launch on
     64-core analysis server (4 * 16 = 64). Adjust if running fewer parallel
     subprocesses.
...
Author: haebarg (2026). Written with AI assistance.
"""
...
# ============================================================
# Halo-profile sensitivity (thesis 6.3) — GCE_PROFILE env switch.
#   GCE_PROFILE = 'NFW2' (default)      -> production NFW^2, NO suffix.
#                                          Byte-identical to run_one_model.py;
#                                          NFW production files are never touched.
#   GCE_PROFILE = 'Einasto2'|'Burkert2' -> alt rho(r) template; '_<profile>'
#                                          suffix on the profile-DEPENDENT files
#                                          ONLY (Extended model XML, Extended
#                                          srcmap, GCE-only XML, GCE gtmodel map,
#                                          .dat/_fit.npz/_likelihood_value).
#   Profile-INDEPENDENT artifacts (ltcube/expcube/ccube/MapCubes and the
#   pion/bremss/ics/bubble/iso gtmodel maps) keep their ORIGINAL names and are
#   SHARED/REUSED across profiles -> same WORK_DIR, zero duplication, and
#   Step 4 _check_or_abort skips the already-built NFW component maps.
#   The alt template ./GCE_template_<profile>.fits must already exist
#   (build it with make_gce_template_profiles.py before launching).
# ============================================================
GCE_PROFILE = (os.environ.get('GCE_PROFILE', 'NFW2').strip() or 'NFW2')
if GCE_PROFILE not in ('NFW2', 'Einasto2', 'Burkert2'):
    raise ValueError(f"GCE_PROFILE must be NFW2/Einasto2/Burkert2, got {GCE_PROFILE!r}")
if GCE_PROFILE != 'NFW2':
    WIMP_MAP_PATH = f'./GCE_template_{GCE_PROFILE}.fits'
    PROF_SUFFIX   = f'_{GCE_PROFILE}'
    if not os.path.exists(WIMP_MAP_PATH):
        raise FileNotFoundError(
            f"GCE template {WIMP_MAP_PATH} not found. Build it first:\n"
            f"    python make_gce_template_profiles.py --profile {GCE_PROFILE}")
else:
    PROF_SUFFIX = ''
print(f'[config] GCE_PROFILE={GCE_PROFILE!r} -> template={WIMP_MAP_PATH}, suffix={PROF_SUFFIX!r}', flush=True)
...
out_dat = f'./GCE_model_{model}{front}_17yr_cholis{CASE_SUFFIX}{VAR_SUFFIX}{PROF_SUFFIX}.dat'
if os.path.exists(out_dat):
    ...
    for _ext in ['_fit.npz', '_likelihood_value']:
        _companion = f'./GCE_model_{model}{front}_17yr_cholis{CASE_SUFFIX}{VAR_SUFFIX}{PROF_SUFFIX}{_ext}'
...
_xml_path = f'{WORK_DIR}/Model/GC_Extended_model{model}{PROF_SUFFIX}_test.xml'
_label = f'GC_Extended_model{model}{PROF_SUFFIX}_test.xml (Step 2)'
...
for convol_setting, convol_suffix in [('yes', ''), ('no', '_no_convol')]:
    _srcmap_out = f'{WORK_DIR}/GC_Extended_srcmap_17yr{front}_clean_model_{model}{PROF_SUFFIX}{convol_suffix}.fits'
    ...
    srcMaps['srcmdl']  = f'{WORK_DIR}/Model/GC_Extended_model{model}{PROF_SUFFIX}_test.xml'
...
for convol_setting, convol_suffix in [('yes', ''), ('no', '_no_convol')]:
    for component in ['pion', 'bremss', 'ics']:
        ...
        gtmodel['srcmaps'] = f'{WORK_DIR}/GC_Extended_srcmap_17yr{front}_clean_model_{model}{PROF_SUFFIX}{convol_suffix}.fits'
        gtmodel.run()
...
for _src_name, _src_xml in _src_specs.items():
    _psfx = PROF_SUFFIX if _src_name == 'GCE' else ''
    _src_model_xml = f'{WORK_DIR}/Model/GC_{_src_name}_model{_psfx}.xml'
    _label = f'GC_{_src_name}_model{_psfx}.xml (Step 5, model-indep template)'
...
for _convol_setting, _convol_suffix in [('yes', ''), ('no', '_no_convol')]:
    _src_srcmap = f'{WORK_DIR}/GC_Extended_srcmap_17yr{front}_clean_model_{model}{PROF_SUFFIX}{_convol_suffix}.fits'
    for _comp_name in ['GCE', 'fermi_bubble', 'isotropic']:
        _psfx = PROF_SUFFIX if _comp_name == 'GCE' else ''
        _comp_out = f'{WORK_DIR}/GC_{_comp_name}_model_17yr{front}_clean{_psfx}{_convol_suffix}.fits'
        ...
        _gtm['irfs']      = IRFS
        _gtm['outtype']   = 'ccube'
        _gtm['srcmdl']    = f'{WORK_DIR}/Model/GC_{_comp_name}_model{_psfx}.xml'
...
GCE    = _mask_avg(f'{WORK_DIR}/GC_GCE_model_17yr{front}_clean{PROF_SUFFIX}_no_convol.fits')
...
class Likelihood:
    def __init__(self, model, energy_bin):
        ...
        self.GCE    = fits.open(f'{WORK_DIR}/GC_GCE_model_17yr{front}_clean{PROF_SUFFIX}.fits')[0].data[energy_bin, 100:500, 100:500]
...
np.savetxt(f'./GCE_model_{model}{front}_17yr_cholis{CASE_SUFFIX}{VAR_SUFFIX}{PROF_SUFFIX}_likelihood_value', max_likelihood)

# .npz — shape (5, n) for visualization-notebook compatibility
np.savez(
    f'./GCE_model_{model}{front}_17yr_cholis{CASE_SUFFIX}{VAR_SUFFIX}{PROF_SUFFIX}_fit.npz',
