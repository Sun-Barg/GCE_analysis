...
evtype_number  = 1
IRFS           = 'P8R3_CLEAN_V3'
# MapCube 소스. run_one_roi_cov.py 와 같은 레이아웃을 쓴다.
# 확장자는 DR4 트랙이 .gz 로 다르다 (프로덕션은 .fits) — 유지.
MAPCUBE_DIR_PION   = os.environ.get('MAPCUBE_DIR', './MapCubes_wide')
MAPCUBE_DIR_BREMSS = MAPCUBE_DIR_PION
MAPCUBE_DIR_ICS    = MAPCUBE_DIR_PION
MAPCUBE_EXT        = '.gz'
...
out_dat = f'./GCE_cov_l{roi}{front}_17yr_cholis_DR4.dat'
...
class Likelihood:
    def __init__(self, model, energy_bin, roi):
        ...
        _psc_mask = np.load(f'{WORK_DIR}/Model/GC_mask_60x60_definitions_DR4.npy')[energy_bin, 100:500, 100:500]
        _disk     = disk_mask
...
def run_mcmc_for_bin(energy_bin):
    ...
    # serial emcee — Pool removed (fermitools fork-unsafe -> SIGKILL)
...
np.savetxt(f'./GCE_cov_l{roi}{front}_17yr_cholis_DR4_likelihood_value', max_likelihood)

np.savez(
    f'./GCE_cov_l{roi}{front}_17yr_cholis_DR4_fit.npz',
    roi                  = roi,
