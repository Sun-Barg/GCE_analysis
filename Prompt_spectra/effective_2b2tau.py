...
class GammaSpectrumInterpolator:
    ...
    def __init__(self, target_mass: float,
                 base_dir: Path = Path("/home/sanghwan/Madgraph_3.5.7/MG5_aMC_v3_5_7/bin/effective_2b2tau"),
                 tag: str = "gamma_v1"):
        ...
        # --------------------------------------------------------------
        dm_masses = np.loadtxt(self.base / "dm_mass.txt", dtype=float)
    ...
    # ------------------------------------------------------------------
    def _file_of(self, m):
        fn = self.base / f"Events/DM{m}GeV/DM{m}GeV_{self.tag}.dat"
        if not fn.exists():
...
if __name__ == "__main__":
    Es, dNdE = GammaSpectrumInterpolator(15).table()
    print(Es[:5])
    print(dNdE[:5])
